 ...
 * =====================================================================================
 *
 *       Filename:  bt_main_network.c
 *
 *    Description:  implemention of bittorrent network interface
 *
 *        Version:  1.0
 *        Created:  09/15/11 23:41:34
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static void __find_unused_sock(
    net_t * net,
    int *peerid
)
{
 ...
 * */
static int __add_new_peer_with_sock(
    net_t * net,
    const int fd
)
{
...
static void __add_to_sendbuffer(
    cbuf_t * cb,
    const int peerid,
    const int size,
    const char *data
...
//    printf("sending: %d %s\n", len, send_data);
    ...
    while (!cbuf_is_empty(net->cb))
    {
        ...
        /*  poll header */
        hdr = cbuf_poll(net->cb, (int) sizeof(buffered_data_header_t));
        /*  poll header + data */
        data = cbuf_poll(net->cb, hdr->size + sizeof(buffered_data_header_t)) +
        ...
        if (0 < diff)
        {
            ...
            cbuf_poll_release(net->cb,
                              sizeof(buffered_data_header_t) + bytes_sent -
                              sizeof(buffered_data_header_t));
            hdr = (void *) cbuf_poll(net->cb, sizeof(buffered_data_header_t));
            hdr->sendee = sendee;
            hdr->size = diff;
            break;
        }
...
//    printf("nsocks_rdy %d\n", nsocks_rdy);
    ...
    if (FD_ISSET(net->peer_listen_sock, &fdset_read))
    {
        ...
        int port = ntohs(cli_addr.sin_port);    // ntohs

        unsigned char *val = (void *) &cli_addr.sin_addr.s_addr;
